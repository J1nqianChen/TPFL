...
def frost(x, severity=1):
    ...
    filename = ['frost_img/frost1.png', 'frost_img/frost2.png', 'frost_img/frost3.png', 'frost_img/frost4.jpg',
                'frost_img/frost5.jpg', 'frost_img/frost6.jpg'][idx]
...
logging.info('Using CIFAR-10 data')
...
try:
    ...
    dataset_name = 'Cifar100_NonIID_Dir0.05_Client20'
    base_dir = '/root/MINE_FL/PFL/dataset'
    total_path = os.path.join(base_dir, dataset_name, 'test')
    logging.info(os.listdir(total_path))
    num_clients = 20
    for client_id in tqdm(range(num_clients)):
        logging.info(f'Creating Corruption for client {client_id}')
        test_x_list, test_y_list = [], []

        for fold in range(1, 4):  # 三折交叉验证
            test_npz_path = os.path.join(total_path, f'{client_id}_{fold}.npz')

            test_arr = np.load(test_npz_path, allow_pickle=True)['data'].tolist()
            x_part = test_arr['x'].transpose(0, 2, 3, 1)  # NCHW -> NHWC
            y_part = test_arr['y']

            test_x_list.append(x_part)
            test_y_list.append(y_part)
        ...
        for method_name in d.keys():
            logging.info('\tCreating images for the corruption', method_name)
            cifar_c, labels = [], []
            ...
            corrupted_x = np.array(cifar_c)
            corrupted_y = np.array(labels)

            save_method_name = method_name.replace(" ", "_")
            save_dir = f'{dataset_name}_C'
            if not os.path.exists(save_dir):
                os.mkdir(save_dir)
            test_path = f'{client_id}_{save_method_name}.npz'
            test_dict = {'x': corrupted_x,
                         'y': corrupted_y}
            with open(os.path.join(save_dir, test_path), 'wb') as f:
                np.savez_compressed(f, data=test_dict)

    send_serverchan('Success', 'Cifar100 c生成完毕')
except Exception as e:
    send_serverchan('Fail', 'e')
