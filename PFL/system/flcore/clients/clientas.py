...
class clientAS(Client):
    ...
    def train(self, is_selected):
        if is_selected:
            ...
            self.model.eval()
            # logging.info(f'client{self.id}, start cal fim.')
            ...
            # logging.info(f"Client {self.id}, Test Accuracy: {test_acc:.1f}, FIM-T value: {fim_trace_sum.item():.1f}")
            # logging.info(f"Selected: {is_selected}, FIM-T value change: {(self.fim_trace_history[-1] - (self.fim_trace_history[-2] if len(self.fim_trace_history) > 1 else 0)):.1f}")
            ...
            # test_acc = self.evaluate()
            # logging.info(f"Client {self.id}, Test Accuracy: {test_acc:.1f}, FIM-T value: {fim_trace_sum.item():.1f}")
            # logging.info(f"FIM-T value change: {(self.fim_trace_history[-1] - (self.fim_trace_history[-2] if len(self.fim_trace_history) > 1 else 0)):.1f}")
