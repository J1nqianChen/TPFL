...
def EDGE(X, Y, U=10, gamma=[1, 1], epsilon=[0, 0], epsilon_vector='range', eps_range_factor=0.1, normalize_epsilon=True,
    ...
    # logging.info('checkpoint 1, data transformed using TSNE')
    ...
    if epsilon[0] == 0:
        ...
        # logging.info('eps_X_temp, eps_Y_temp', eps_X_temp, eps_Y_temp)
        # Normalizing factors for the bandwidths
        cx, cy = 18 * d_X_shrink / np.max([(1 + 1. * math.log(dim_X)), 1]), 18 * d_Y_shrink / np.max(
            [(1 + 1. * math.log(dim_Y)), 1])
        eps_X0, eps_Y0 = eps_X_temp * cx * gamma[0], eps_Y_temp * cy * gamma[1]
    # logging.info('********eps_X0, eps_Y0', eps_X0, eps_Y0)
    else:
        eps_X_temp = np.ones(d_X_shrink, ) * epsilon[0]
        eps_Y_temp = np.ones(d_Y_shrink, ) * epsilon[1]
        cx, cy = 15 * d_X_shrink / np.max([(1 + 1.0 * math.log(dim_X)), 1]), 15 * d_Y_shrink / np.max(
            [(1 + 1.0 * math.log(dim_Y)), 1])
        eps_X0, eps_Y0 = eps_X_temp * cx * gamma[0], eps_Y_temp * cy * gamma[1]
    # logging.info('eps_X0, eps_Y0', eps_X0, eps_Y0)
    ...
    ## Compute MI Vector
    # logging.info('Compute MI Vector: ')
    # MI Vector
    ...
    for j in range(L_ensemble):
        ...
        # logging.info('j, eps_X, eps_Y', j, eps_X, eps_Y)
        ## Shifts of hashing
        if stochastic == True:
            np.random.seed()
            f = 0.1
