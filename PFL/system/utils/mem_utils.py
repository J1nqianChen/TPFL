...
class MemReporter():
    ...
    def print_stats(self, verbose: bool = False, target_device: Optional[torch.device] = None) -> None:
        ...
        # logging.info(template_format.format('Element type', 'Size', 'Used MEM') )
        for device, tensor_stats in self.device_tensor_stat.items():
            ...
            for stat in tensor_stats:
                ...
                # if not show_reuse:
                #     name = name.split('(')[0]
                # logging.info(template_format.format(
                #     str(name),
                #     str(size),
                #     readable_size(mem),
                # ))
            ...
            logging.info('-' * LEN)
            logging.info('Total Tensors: {} \tUsed Memory: {}'.format(
            ...
            if device != torch.device('cpu'):
                ...
                logging.info('The allocated memory on {}: {}'.format(
                    device, readable_size(memory_allocated),
                ))
                if memory_allocated != total_mem:
                    logging.info('Memory differs due to the matrix alignment or'
